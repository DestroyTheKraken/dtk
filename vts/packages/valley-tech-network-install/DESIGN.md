...
| Interface role | Purpose | Default addressing |
|----------------|---------|-------------------|
| **WAN** | Modem / Starlink / ISP | DHCP client |
| **LAN1 — Trusted** | Family / office PCs, phones | `192.168.10.0/24`, gateway `.1`, **DHCP** |
| **LAN2 — Guest / IoT** | Guests + smart home | `192.168.50.0/24`, gateway `.1`, **DHCP** |
...
- LAN1 → WAN: allow (NAT)  
- LAN2 → WAN: allow (NAT)  
- LAN2 → LAN1: **drop** (no guest/IoT into trusted)  
- LAN1 → LAN2: allow optional (or drop; default **drop** both ways except established)  
- WAN → LANs: drop unsolicited  

**Static IPs:** not in default install. You add DHCP reservations or statics **post-install per client** when needed.

**Interface names:** provisioner asks which NIC is WAN/LAN1/LAN2 (hardware differs). Defaults: first NIC WAN, second LAN1, third LAN2 when three NICs exist; with two NICs, document VLAN or USB-NIC pattern.
