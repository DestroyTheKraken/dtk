...
mkdir -p ~/valley-tech-support/packages/network-install/configs
...
ssh router-lab '/opt/vyatta/bin/vyatta-op-cmd-wrapper show configuration' \
  > ~/valley-tech-support/packages/network-install/configs/config.boot.from-router-LATEST

# 3) Confirm the file has content (should be many lines, not empty)
wc -l ~/valley-tech-support/packages/network-install/configs/config.boot.from-router-LATEST
```
...
Or: `less ~/valley-tech-support/packages/network-install/configs/config.boot.from-router-LATEST`
...
scp -i ~/.ssh/id_ed25519_vyos \
  ...
  ~/valley-tech-support/packages/network-install/configs/config.boot.from-scp
```
