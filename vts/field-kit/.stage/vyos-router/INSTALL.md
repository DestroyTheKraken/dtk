...
## End state (3 Ethernet ports only)

| Port | This build | Network | DHCP |
|------|------------|---------|------|
| WAN | `eth0` built-in | ISP (Starlink/modem) | Client from ISP |
| LAN1 | `eth2` USB | Trusted Family or Staff `192.168.10.0/24` | Server on router |
| LAN2 | `eth3` USB | Guest **+** IoT together `192.168.50.0/24` | Server on router |

`eth1` is not used (not present on this USB-NIC naming).  
Isolation: LAN2 cannot reach LAN1.  
Statics / monitoring: optional later; not in base template.
